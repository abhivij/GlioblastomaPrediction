...
def execute_model(network_function, criterion, device, print_details=False):
	...
	for _ in range(METRIC_COMPUTATION_ITER):
		...
		training_data, test_data = get_data_and_train_model(GBM_PATH, network, criterion, optimiser, device, print_details)
...
def execute_tl_model(network_function, criterion, device, print_details=False):
	...
	for _ in range(METRIC_COMPUTATION_ITER):
		...
		_, _ = get_data_and_train_model(NSCLC_PATH, network, criterion, optimiser, device, print_details)
		training_data, test_data = get_data_and_train_model(GBM_PATH, network, criterion, optimiser, device, print_details)
